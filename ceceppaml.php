...
//         if($lang_id <= 0) return $permalink;
	...
	foreach($plinks as $plink) :
	  ...
	  if(!empty($plink)) :
	    ...
	    if(is_object($id)) :
	      $id = $id->term_id;
	      $cat = ($this->_translate_term_link) ? strtolower(get_option("cml_category_" . $id . "_lang_" . $lang_id, $plink)) : $plink;
	    else:
...
require_once(CECEPPA_PLUGIN_PATH . "test.php");
?>